...
df = pd.DataFrame(columns=["TITLE", "SEASON", "REVIEW_RANK","IMAGE","GAIYOU"])
loginUrl = "https://www.anikore.jp/users/login/"
loginName = "slumdank3@gmail.com"
loginPass = "nishihigashi"
url = r"https://www.anikore.jp/tag/%E3%82%B7%E3%83%AA%E3%82%A2%E3%82%B9/" #シリアスランキング
...
IMAGE = ".lazyload"
GAIYOU = ".animeDescription.naturalFont.fs-10to14"
    ...
    if browser.find_element_by_css_selector(PAGE_NEXT).get_attribute("href") is not None:
        ...
        for post in posts:
            try:
                ...
                if (str(title).find("映画") >= 0) | (str(title).find("OVA") >= 0) \
                    | (str(title).find("OAD") >= 0) | (str(title).find("その他") >= 0) :
                    continue
                print(rank)
                print(title)
                ...
                print(image)
                gaiyou = post.find_element_by_css_selector(GAIYOU).text
                print(gaiyou)
...
#CSV出力
print("CSVの出力")
df.to_csv("scraping/outputSyliusAnime.csv")
print("完了")
